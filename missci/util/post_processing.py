...
def extract_json(text: str) -> Optional[Dict]:
    index_first: int = text.find("{")
    index_last: int = text.rfind("}")

    if min((index_first, index_last)) < 0:
        return None

    else:
        try:
            return json.loads(text[index_first: index_last+1])
        except JSONDecodeError:
            return None


def passage_to_argument_level_predictions(predictions: List[Dict]) -> List[Dict[str, Dict]]:
    argument_to_passages: Dict[str, Dict[str, Dict]] = defaultdict(lambda: defaultdict(dict))
    for prediction in predictions:
        argument_id: str = prediction['data']['argument_id']
        passage_id: str = prediction['data']['passage_id']
        argument_to_passages[argument_id][passage_id] = prediction['score']

    arg_level_predictions = [
        {
            'argument': argument_id,
            'predictions': argument_to_passages[argument_id]
        } for argument_id in argument_to_passages
    ]
    return arg_level_predictions
